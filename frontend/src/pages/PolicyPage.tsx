...
import policyHeroUrl from '../assets/policy-hero.webp'
import policyGeoAiUrl from '../assets/policy-geoai.webp'
import policyHumanEarthUrl from '../assets/policy-human-earth.webp'
import policyRemoteSensingUrl from '../assets/policy-remote-sensing.webp'
import policyOpenDataUrl from '../assets/policy-open-data.webp'
...
interface SciArticle {
  id: string; tag: string; title: string; body: string; icon: React.ReactNode; image: string
}

const SCI_ARTICLES: SciArticle[] = [
  { id: 's1', tag: 'GeoAI 基础', icon: <RocketOutlined />, image: policyGeoAiUrl,
  ...
  { id: 's2', tag: '人地系统', icon: <GlobalOutlined />, image: policyHumanEarthUrl,
  ...
  { id: 's3', tag: '遥感技术', icon: <BulbOutlined />, image: policyRemoteSensingUrl,
  ...
  { id: 's4', tag: '数据开放', icon: <BookOutlined />, image: policyOpenDataUrl,
    title: '开放地理数据：价值与挑战',
...
export default function PolicyPage() {
  ...
  return (
    <div>
      <section className="gs-policy-hero">
        <div className="gs-policy-hero__content">
          <h1>政策科普</h1>
          <p>地理空间数据政策文件解读 &amp; 地理空间智能科学知识宣传，助力理解技术、把握政策、探索应用。</p>
        </div>
        <div className="gs-policy-hero__visual" aria-hidden="true">
          <img src={policyHeroUrl} alt="" />
        </div>
      </section>
        ...
        <div className="gs-sci-grid">
          {SCI_ARTICLES.map((a) => (
            <article key={a.id} className="gs-sci-card">
              <div className="gs-sci-card__media">
                <img src={a.image} alt="" />
              </div>
              <div className="gs-sci-card__content">
                <Tag className="gs-sci-card__tag">{a.tag}</Tag>
                <h3 className="gs-sci-card__title">{a.title}</h3>
                <p className="gs-sci-card__body">{a.body}</p>
                <button type="button" className="gs-sci-card__more">
